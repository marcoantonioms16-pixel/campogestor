...
async function sbSignup(email, password) {
  throw new Error("Cadastro público está desligado. Use a conta da fazenda.");
}
...
async function sbLogin(email, password) {
  ...
  if (pulled === true) {
    ...
    // Só envia o aparelho se já houver cadastro local. Não sobe semente vazia.
    const temDadosLocais = Array.isArray(state.pessoas) && state.pessoas.length > 0;
    if (temDadosLocais) await pushCloud();
    else cloudStatus = "synced";
  } else {
