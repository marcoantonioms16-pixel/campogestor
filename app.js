...
function renderDrawer(){
  ...
  if(edit.kind==="farm"){
    body=`<h2>Editar fazenda</h2>
      ...
      <div class="field"><label>WhatsApp Clara (DDD + número)</label><input id="f-wa" placeholder="62999999999" value="${esc(state.farm.whatsappClara||"")}"/></div>
...
function renderProfileModal(){
  ...
  return `<div class="profile-overlay" id="profile-overlay" role="dialog" aria-modal="true" aria-label="Perfil do usuário">
    <section class="profile-modal">
      ...
      `<div class="profile-auth"><p class="muted">Sessão encerrada. Entre de novo para ver os dados da fazenda.</p></div>`}
    </section>
  </div>`;
}
...
function renderLoginGate(){
  const offline = (typeof isAppOnline === "function" && !isAppOnline());
  return `<div class="login-gate">
    <div class="login-card">
      <div class="login-brand">CAMPO<span>GESTOR</span></div>
      <p class="login-sub">Fazenda Santa Rita</p>
      <p class="muted">${offline?"Sem internet. Conecte-se para entrar.":"Acesso só com a conta da fazenda."}</p>
      <div class="field"><label>E-mail</label><input id="auth-email" type="email" placeholder="seu@email.com" autocomplete="username" ${offline?"disabled":""}/></div>
      <div class="field"><label>Senha</label><input id="auth-pass" type="password" placeholder="••••••" autocomplete="current-password" ${offline?"disabled":""}/></div>
      <button type="button" class="btn primary block" id="btn-login" ${offline?"disabled":""}>Entrar</button>
      <p class="login-foot muted">Não há cadastro público. Peça acesso ao administrador.</p>
    </div>
  </div>`;
}
...
function bind(){
  ...
  if(bwa) bwa.onclick=(ev)=>{
    ev.preventDefault();
    let num=(state.farm.whatsappClara||"").replace(/\D/g,"");
  ...
  if(br) br.onclick=()=>{
    if(!confirm("Restaurar o cadastro-base do app? Lançamentos deste aparelho podem ser apagados.")) return;
    state=JSON.parse(JSON.stringify(SEED)); state.rotinaFeita={}; save(); toast("Cadastro-base restaurado"); render();
  };
  bindAuth();
}
